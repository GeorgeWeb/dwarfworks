...
class DW_API RenderCommand {
 public:
  ...
  inline static void SetScissor(uint32_t width, uint32_t height) {
  ...
  inline static void SetBlending(bool enable = true) {
    s_RendererAPI->SetBlending(enable);
  }

  inline static void SetDepthTest(bool enable = true) {
    s_RendererAPI->SetDepthTest(enable);
  }

  inline static void SetScissorTest(bool enable = true) {
    s_RendererAPI->SetScissorTest(enable);
  }
  ...
  inline static void Clear(uint32_t target = BufferTarget::Color) {
    s_RendererAPI->Clear(target);
  }
