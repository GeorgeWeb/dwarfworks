...
Playground::Playground()
  ...
  triangleVB =
      Dwarfworks::VertexBuffer::Create(triangleVertices, tiangleVbSize);
  ...
  // vertex buffer layout
  squareVB->SetLayout({{Dwarfworks::ShaderDataType::Float3, "a_Position"},
  ...
  // purposes because UBOs in OpenGL/Vulkan are not the same in Direct3D -
  // they are Constant buffers.
  ...
  // Load texture(s)
  m_Textures["checkerboard"] =
      Dwarfworks::Texture2D::Create("Assets/Textures/Checkerboard.png");
...
void Playground::OnUpdate(Dwarfworks::Timestep deltaTime) {
  // Poll user input
  // ---------------
...
void Playground::OnRender() {
  ...
  for (int y = 0; y < 20; y++) {
    for (int x = 0; x < 20; x++) {
      ...
      flatColorShader->SetFloat4("u_Color",
                                 (x % 2 == 0) ? blueColor : redColor);
...
void Playground::OnDebugUIRender() {
...
void Playground::OnEvent(Dwarfworks::Event& event) {
  ...
  if (event.GetEventType() == Dwarfworks::EventType::WindowResize) {
    ...
    m_CameraController.GetCamera().SetProjection(-aspectRatio * zoomLevel,
